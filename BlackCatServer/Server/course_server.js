...
// 获取各个时段可以预约教练的数量
var getUseFullCoachCount=function(schoolid,_date,callback){
    coursemode.aggregate([
        ...
        {$match:{
            ...
            ,{$group:{_id:"$coursetime.timeid",coachcount : {$sum : 1}}}
        ],
        function(err,data) {
            return callback(err,data)
        })
}
//getUseFullCoachCount("562dcc3ccb90f25c3bde40da",new Date("2016-03-17"),function(err,data){
//    console.log(data);
//})
// 2.0 预约界面
exports.getMyCourseoneday=function(coachid,userid,date ,callback){
    basedatafun.getstudentinfo(userid,function(err,userdata){
        var datenow = new Date( new Date(date).toFormat("YYYY-MM-DD").toString());
        getUseFullCoachCount(userdata.applyschool,datenow,function(err,coursecount){
        ...
        coachmode.findById(new mongodb.ObjectId(reservationinfo.coachid), function (err, coachdata) {
            ...
            VerificationCourse(arr, reservationinfo.userid, function (err) {
                ...
                if (userdata.subject.subjectid == 2) {
                    ...
                    //判断用户预约课程数量
                    if (userdata.subjecttwo.finishcourse+userdata.subjecttwo.reservation
                        + coursecount >= userdata.subjecttwo.totalcourse) {
                    ...
                    if (userdata.subjectthree.finishcourse+userdata.subjectthree.reservation+
                         coursecount >= userdata.subjectthree.totalcourse) {
                        return callback("您的课程已约满");
                    }
...
exports.getuserReservation=function(userid,subjectid,reservationstate,callback){
    ...
    reservationmodel.find(searhinfo)
        .select("coachid reservationstate reservationcreatetime subject shuttleaddress classdatetimedesc " +
        "courseprocessdesc trainfieldlinfo  is_comment sigintime  begintime endtime  cancelreason learningcontent")
