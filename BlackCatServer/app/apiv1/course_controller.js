...
require('date-utils');
exports.getcoursebycoachv2=function(req,res){
    ...
    courseserver.getMyCourseoneday(coachid,userid,date,function(err,data){
        if (err){
...
exports.postReservation=function(req,res){
    ...
    if(reservationinfo.userid!=req.userId&&reservationinfo.coachid!=req.userId){
        return res.json(
            new BaseReturnInfo(0,"无法确认请求用户",""));
    };
    ...
    var now=new Date();
    if(now-(new Date(reservationinfo.begintime))>0){
...
//  教练评价 v1.1 版本
exports.coachCommentV2=function(req,res){
    var commentinfo= {
        coachid:req.body.coachid,
        reservationid:req.body.reservationid,
        starlevel:req.body.starlevel,
    ...
    courseserver.coachCommentV2(commentinfo,function(err,data){
        if (err){
            return res.json(new BaseReturnInfo(0,err,""));
        }
        return res.json(new BaseReturnInfo(1,"",data));
...
// 获取教练某一课程预约列表
exports.getCoursereservationlist=function(req,res){
    var  coachid=req.query.coachid;
    var  courseid=req.query.courseid;
    if (coachid===undefined|| courseid===undefined){
        return res.json(new BaseReturnInfo(0,"获取参数错误",[]));
    }
    ...
    courseserver.getCoursereservationlist(coachid,courseid,function(err,data){
        if (err){
            return res.json(new BaseReturnInfo(0,err,[]));
        }
        return res.json(new BaseReturnInfo(1,"",data));
