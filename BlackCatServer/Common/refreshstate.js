...
var sendsms=require("./sendsmscode");
var pushmsg=require("./PushStudentMessage");
        ...
        async.whilst(
            ...
            function(cb) {
                ...
                reservationmodel.findOneAndUpdate({reservationstate:appTypeEmun.ReservationState.applyconfirm,
                    ...
                    { $set: { is_sendsms:true,is_sendmessage:true }}
                    ,function(err,doc){
                        if(err){
                            cb(err);
                        }
            ...
            function(err) {
                // 3s have passed
                if(err) {
                    console.log('提醒上课信息: ', err);
                }
            }
        );
     });
} catch(e){
    console.log(new Date().toString()+'发送短信上课提醒error..'+ e.message);
}
